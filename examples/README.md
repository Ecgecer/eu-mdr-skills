...
> These transcripts were produced by an earlier version of the skill, before a
> repo-wide punctuation change on 2026-09-11 replaced em dashes throughout the prose.
> They are regenerated from a stored eval run by `scripts/build-example.py` and never
> edited by hand. They will be rebuilt from the next measured run.
