...
> Regenerated by `scripts/build-example.py` from the stored run
> `2026-09-17T18-43-23-939Z` (case `hwg11-item-scope`, Claude Code 2.1.274), and
> never edited by hand. The
> generator refuses to run if the skill was edited after the run it draws from, so
> these transcripts always describe the text this repo currently ships.
